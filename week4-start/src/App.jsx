...
export default function App() {
  ...
  // 지금 단어장에 들어 있는 단어 이름 목록입니다.
  // 사전 검색 결과와 추천 단어 양쪽에서 "이미 있는지" 판단할 때 씁니다.
  const addedWords = words.map((item) => item.word)

  // 이미 단어장에 있는 단어인지 확인합니다.
  const alreadyAdded = result !== null && addedWords.includes(result.word)

  /**
   * 단어 하나를 내 단어장 맨 앞에 넣습니다.
   * 사전 검색 결과도, 미니 API 의 추천 단어도 결국 같은 모양이라 함수 하나로 처리됩니다.
   *
   * id 는 여기서 Date.now() 로 새로 발급합니다.
   * 미니 API 가 준 'api-proxy' 같은 id 를 그대로 쓰면
   * 같은 단어를 다시 넣었을 때 key 가 겹칠 수 있기 때문입니다.
   */
  function addWord(entry) {
    if (addedWords.includes(entry.word)) return
  ...
  return (
    <>
      ...
      {/* 미니 API 에서 받아오는 추천 단어입니다.
          이 요청이 어디로 나가는지는 .env 의 VITE_API_URL 이 정합니다.
          비워두면 상대경로 /api/words 로 나가서 nginx 프록시를 타게 됩니다. */}
      <RecommendedWords onAdd={addWord} addedWords={addedWords} />
